...
public class CorrectedBill {
	...
	// Bill State Machines
	enum BillState {
		inPreparation, inHouseOfCommons, inSenate, awaitingRoyalAssent, actOfParliament, withdrawn
	}
	...
	enum BillStateInSenate {
		Null, firstReadingS, secondReadingS, committeeConsiderationS, thirdReadingS
	}
	...
	private void setBillState(BillState aBillState) {
		...
		switch (billState) {
		case inHouseOfCommons:
			if (billStateInHouseOfCommons == BillStateInHouseOfCommons.Null) {
				setBillStateInHouseOfCommons(BillStateInHouseOfCommons.firstReading);
			}
			break;
		case inSenate:
			if (billStateInSenate == BillStateInSenate.Null) {
	...
	private void setBillStateInHouseOfCommons(BillStateInHouseOfCommons aBillStateInHouseOfCommons) {
		...
		if (billState != BillState.inHouseOfCommons && aBillStateInHouseOfCommons != BillStateInHouseOfCommons.Null) {
			setBillState(BillState.inHouseOfCommons);
		}
	}
	...
	private void setBillStateInSenate(BillStateInSenate aBillStateInSenate) {
		...
		if (billState != BillState.inSenate && aBillStateInSenate != BillStateInSenate.Null) {
			setBillState(BillState.inSenate);
		}
	}
	...
	public String toString() {
		String outputString = "";
		return super.toString() + "[" + "isCommonsBill" + ":" + getIsCommonsBill() + "]" + outputString;
	}
}